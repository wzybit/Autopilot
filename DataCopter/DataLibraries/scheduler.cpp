...
void BIT_Scheduler::init(const BIT_Scheduler::Task *tasks, uint8_t num_tasks)
{
    ...
    _loop_rate_hz = 1000; // 1000hz 主循环是1ms  一次 最大频率目前限制为1000hz
...
void BIT_Scheduler::run(uint32_t time_available)
{
    for (uint8_t i=0; i<_num_tasks; i++)
    {
        ...
        if (dt >= interval_ticks)
        {
            ...
            //DEBUG_PRINTF("_tasks[%d].max_time_micros = %d \n", i, _tasks[i].max_time_micros);

            if (dt >= interval_ticks * 2)
            {
                DEBUG_PRINTF("Scheduler slip task[%u] \n",(unsigned)i);// 说明应该执行该任务的但是错过了一次
            }

            if (_task_time_allowed <= time_available)
            {

                _task_time_started = (uint64_t)clock_gettime_us();
                _tasks[i].function();
                //(*_tasks[i].function)();//wzy C++ advocated
                /*
                ...
                // 计算这个任务到底需要花了多少时间
                uint16_t time_taken = (uint64_t)clock_gettime_us() - _task_time_started;
                globalSystemTime.time_loop_scheduler_us[i] = time_taken;
                //DEBUG_PRINTF("task [%2d]  time_taken    : =%d \n", i, time_taken);

                if (time_taken > _task_time_allowed)
                {
                    DEBUG_PRINTF("Scheduler overrun task[%2u], time_taken is:%d \n", (unsigned)i, time_taken);
                    return;
                }
