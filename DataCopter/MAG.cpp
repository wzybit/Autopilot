...
{	
	...
	for(u8 i = 0;i<3;i++)
	{
		globalSaveData.mag_gain[i] = LIMIT(globalSaveData.mag_gain[i],0.05f,100);
		mag.val[i] = (mag_in[i] - globalSaveData.mag_offset[i]) * globalSaveData.mag_gain[i];
	}
	...
	{	
		switch(mag_cal_step)
		{
			...
			case 5:
				for(u8 i = 0;i<3;i++)
				{
					globalSaveData.mag_offset[i] = 0.5f *(max_t[i] + min_t[i]);		//中值校准
					globalSaveData.mag_gain[i] = safe_div(200.0f ,(0.5f *(max_t[i] - min_t[i])),0);		//幅值校准
				}
